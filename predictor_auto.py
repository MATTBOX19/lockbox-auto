#!/usr/bin/env python3
# predictor_auto.py — LockBox Pro ATS/OU Adaptive Predictor (with self-learning + per-sport tuning)
...
# --- Adaptive tuning based on metrics.json ---
if METRICS_FILE.exists():
    try:
        ...
        if isinstance(metrics, list) and metrics:
            recent = metrics[-1]
            rates = {
                "ML": recent.get("ml_win_pct", 0),
                "ATS": recent.get("ats_win_pct", 0),
                "OU": recent.get("ou_win_pct", 0)
            }
            best_type = max(rates, key=rates.get)
            for t in ["ML", "ATS", "OU"]:
                if t == best_type:
                    cfg[f"{t}_WEIGHT"] = round(min(1.4, cfg.get(f"{t}_WEIGHT", 1.0) * 1.05), 3)
                else:
                    cfg[f"{t}_WEIGHT"] = round(max(0.7, cfg.get(f"{t}_WEIGHT", 1.0) * 0.98), 3)
            cfg["_last_adaptive_update"] = datetime.utcnow().isoformat()
            with open(CONFIG_FILE, "w") as f:
                json.dump(cfg, f, indent=2)
            print(f"🧠 Adaptive tuning: favored {best_type} ({rates[best_type]}%) → {cfg[best_type + '_WEIGHT']}x")
    except Exception as e:
...
def nfl_stat_prob(t1, t2):
    ...
    if not a1 or not a2 or a1 not in NFL_STATS.index or a2 not in NFL_STATS.index:
        return None, None
            ...
            if mk.get("spreads"):
                ...
                if len(outs_sp) == 2:
                    ...
                    if s1 is not None and s2 is not None:
                        ...
                        edge_ats = abs(t1_diff - t2_diff) * cfg["ATS_WEIGHT"]
...
if not rows:
    ...
    print("🚀 Done — LockBox Pro adaptive model ready for web display.")
